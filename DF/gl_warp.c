...
void R_InitUnderwaterWarpTexture (void)
{
	...
	// also BE VERY CAREFUL not to specify any TEXPREF_ flags that will modify the data in-place or we'll scribble over prior allocations.
	r_underwaterwarptexture = TexMgr_LoadImage (NULL, "", vid.maxwarpwidth, vid.maxwarpheight, SRC_INDEXED, hunk_base, "", (unsigned) hunk_base, TEXPREF_NOPICMIP | TEXPREF_PERSIST | TEXPREF_LINEAR);
	...
	glCopyTexSubImage2D (GL_TEXTURE_2D, 0, 0, 0, 0, 0, vid.maxwarpwidth, vid.maxwarpheight);
}


void D_CalcUnderwaterCoords (float x, float y, float CYCLE_X, float CYCLE_Y, float AMP_X, float AMP_Y)
{
...
void D_WarpScreen (void)
{
	...
	for (x = 0; x < 32; x++)
	{
		...
		for (y = 0; y <= 32; y++)
		{
			D_CalcUnderwaterCoords ((float) x / 32.0f, (float) y / 32.0f, CYCLE_X, CYCLE_Y, AMP_X, AMP_Y);
			D_CalcUnderwaterCoords ((float) (x + 1) / 32.0f, (float) y / 32.0f, CYCLE_X, CYCLE_Y, AMP_X, AMP_Y);
		}
