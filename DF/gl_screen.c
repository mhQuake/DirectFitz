...
#define ADJUST_CENTERPRINT_POSITIONS
...
#ifdef ADJUST_CENTERPRINT_POSITIONS
...
#ifdef ADJUST_CENTERPRINT_POSITIONS
	y = (vid.height * 0.35) - (scr_center_lines * 4);
#else
	...
	// scan the width of the line
		for (l=0 ; l<40 ; l++)
			if (start[l] == '\n' || !start[l])
				break;
#ifdef ADJUST_CENTERPRINT_POSITIONS
		x = (glwidth - l*8)/2;	//johnfitz -- 320x200 coordinate system
#else
...
void SCR_SetUpToDrawConsole (void)
{
	...
	// get correct frametime irrespective of timescale/etc
	...
	if (con_forcedup)
	{
		scr_conlines = glheight;		// full screen
		scr_con_current = scr_conlines;
	}
	else if (key_dest == key_console)
		scr_conlines = glheight / 2;	// half screen
	else
		scr_conlines = 0;				// none visible

	// adjust these properly relative to video height so that the console doesn't scroll slower on larger screens
	if (scr_conlines < scr_con_current)
	{
		scr_con_current -= scr_conspeed.value * (glheight / 200.0f) * con_frametime;
		if (scr_conlines > scr_con_current)
			scr_con_current = scr_conlines;
...
void SCR_ScreenShot_f (void)
{
	...
	for (i = 0; i < 10000; i++)
	{
		sprintf (tganame, "fitz%04i.jpg", i);
	...
	Direct3D9_ScreenShotJPG (checkname);
