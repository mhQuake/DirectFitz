...
*/
#ifdef EXTRA_DLIGHTS
dlight_t *CL_AllocDlight (int key, int r, int g, int b)
#else
...
// rotate binary objects locally
		...
		{
			dl = CL_AllocDlight (i, DL_COLOR_WHITE);
			VectorCopy (ent->origin,  dl->origin);
			dl->origin[2] += 16;
		...
		if (ent->effects & EF_DIMLIGHT)
		{
			...
			if (i == cl.viewentity)
			{
				...
				if ((cl.items & IT_QUAD) && (cl.items & IT_INVULNERABILITY))
					...
					dl = CL_AllocDlight (i, DL_COLOR_RED);
				else dl = CL_AllocDlight (i, DL_COLOR_WHITE);
			}
			else dl = CL_AllocDlight (i, DL_COLOR_WHITE);
#else
			dl = CL_AllocDlight (i);
