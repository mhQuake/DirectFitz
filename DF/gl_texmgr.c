...
#ifdef TXGAMMA
static void TexMgr_CheckTxGamma (unsigned char *palette)
{
	int	i;

	// this must be initialized in case gamma was not specified otherwise it would be 0 and 32-bit textures would be black
	for (i = 0; i < 256; i++)
		tx_gammatable[i] = i;

	if ((i = COM_CheckParm ("-txgamma")) != 0)
	{
		float txgamma = Q_atof (com_argv[i + 1]);

		// build the gamma table for 8-bit and 32-bit textures
		for (i = 0; i < 256; i++)
		{
			float f = pow ((i + 1) / 256.0, txgamma);
			float inf = f * 255 + 0.5;
		...
		// apply the gamma table to the palette as it is read from disk so that it will apply to all generated palettes
		for (i = 0; i < 768; i++)
		{
			palette[i] = tx_gammatable[palette[i]];
		}
	}
}


byte *TexMgr_ApplyTxGamma32 (byte *data, int size)
{
	int i;

	/*
	Here's a great example of where FitzQuake can trip you up.

	We cannot apply gamma in-place because FitzQuake passes hunk_base as a data parameter to texture loading
	calls for it's warp images.  Applying gamma in-place would therefore stomp over previous memory allocations.

	HOWEVER
	OpenGL allows you to pass a NULL pointer to glTexImage2D and it will allocate the texture without data, so
	FitzQuake's behaviour is completely unnecessary.  HOWEVER it's not so straightforward to rewrite it's loader
	to allow for NULL pointers because it has assumptions that the data pointer will be non-NULL all over the place.

	And of course there are loads of other 32-bit texture loading functions that do operate in-place but the TEXPREF
	flags used ensure that these ones are safe.

	I hit this kind of crap ALL THE TIME when working on RMQ and it used to drive me nuts.

	At one stage I experienced a physical reaction to the sight of the "TexMgr_" prefix and wasted a lot of time
	renaming all of the functions to "TM_" instead.
	*/
	byte *in = data;
	byte *out = (byte *) Hunk_Alloc (size * 4);

	data = out;

	for (i = 0; i < size; i++, in += 4, out += 4)
	{
		// gamma is not applied to the alpha byte
		out[0] = tx_gammatable[in[0]];
		out[1] = tx_gammatable[in[1]];
		out[2] = tx_gammatable[in[2]];
		out[3] = in[3];
	}
...
#ifdef COLOR_255_IS_NOT_FULLBRIGHT
	d_8to24table_fbright[255] = d_8to24table[255];
#endif
...
#ifdef COLOR_255_IS_NOT_FULLBRIGHT
	d_8to24table_nobright[255] = d_8to24table[255];
#endif
...
void TexMgr_LoadImage32 (gltexture_t *glt, unsigned *data)
{
	...
	// the d3d wrapper takes over miplevel generation so doing it in the engine too is wasteful
							   ...
							   byte *data, char *source_file, unsigned source_offset, unsigned flags)
{
	...
	switch (glt->source_format)
	{
		...
		case SRC_RGBA:
#ifdef TXGAMMA
			TexMgr_LoadImage32 (glt, (unsigned *) TexMgr_ApplyTxGamma32 (data, width * height));
#else
			TexMgr_LoadImage32 (glt, (unsigned *)data);
#endif
			break;
