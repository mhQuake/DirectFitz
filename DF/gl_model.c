...
qboolean Mod_CheckFullbrights (byte *pixels, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		// 255 is not fullbright
		if (pixels[i] == 255) continue;
		if (pixels[i] > 223) return true;
	}
...
//johnfitz
	...
	for (i=0 ; i<nummiptex ; i++)
	{
		...
		if (!isDedicated) //no texture uploading for dedicated server
		{
			...
			{
#ifdef ALPHAMASK_TEXTURES
				int alphaflag = (tx->name[0] == '{') ? TEXPREF_ALPHA : 0;
#endif
				...
				if (data) //load external image
				{
					...
					tx->gltexture = TexMgr_LoadImage (loadmodel, filename, fwidth, fheight,
						SRC_RGBA, data, filename, 0, TEXPREF_MIPMAP | alphaflag);
#else
					tx->gltexture = TexMgr_LoadImage (loadmodel, filename, fwidth, fheight,
				...
				else //use the texture from the bsp file
				{
					...
					if (Mod_CheckFullbrights ((byte *)(tx+1), pixels))
					{
#ifdef ALPHAMASK_TEXTURES
						tx->gltexture = TexMgr_LoadImage (loadmodel, texturename, tx->width, tx->height,
							SRC_INDEXED, (byte *)(tx+1), loadmodel->name, offset, TEXPREF_MIPMAP | TEXPREF_NOBRIGHT | alphaflag);
						...
						tx->fullbright = TexMgr_LoadImage (loadmodel, texturename, tx->width, tx->height,
							SRC_INDEXED, (byte *)(tx+1), loadmodel->name, offset, TEXPREF_MIPMAP | TEXPREF_FULLBRIGHT | alphaflag);
#else
						tx->gltexture = TexMgr_LoadImage (loadmodel, texturename, tx->width, tx->height,
...
#ifdef ALPHAMASK_TEXTURES
						tx->gltexture = TexMgr_LoadImage (loadmodel, texturename, tx->width, tx->height,
							SRC_INDEXED, (byte *)(tx+1), loadmodel->name, offset, TEXPREF_MIPMAP | alphaflag);
#else
						tx->gltexture = TexMgr_LoadImage (loadmodel, texturename, tx->width, tx->height,
...
{
#ifdef MOD_LOAD_LIGHTING_IS_INSANE
	int i;
	byte *in, *out, *data;
	char litfilename[1024];
	int mark = Hunk_LowMark ();

	// if there's no light in the BSP then we won't have the face data to use a LIT file either, so here is the correct place for this check
	if (!l->filelen) return;

	// LordHavoc: check for a .lit file
	// MH - I'm really not happy about LordHavoc's aversion to whitespace
	strcpy (litfilename, loadmodel->name);
	COM_StripExtension (litfilename, litfilename);
	strcat (litfilename, ".lit");

	if ((data = (byte*) COM_LoadHunkFile (litfilename)) != NULL)
	{
		// MH - the LIT file size should be exactly 3x the lump size, adding 8 bytes for the header, otherwise the LIT file was made for a different map with the same name
		if (data[0] == 'Q' && data[1] == 'L' && data[2] == 'I' && data[3] == 'T' && com_filesize == (l->filelen * 3) + 8)
		{
			if ((i = LittleLong (((int *) data)[1])) == 1)
			{
	...
	// if the lit file was bad free it now
	Hunk_FreeToLowMark (mark);

	// LordHavoc: no .lit found, expand the white lighting data to color
	// MH - I'm not certain what LordHavoc was thinking/smoking here, but his original code actually passed over the light data twice - once for the
	// memcpy and once for the expansion.  Only the second pass was actually necessary if we set up if the "in" pointer correctly.
	loadmodel->lightdata = (byte *) Hunk_AllocName ( l->filelen * 3, litfilename);
	in = &mod_base[l->fileofs];
	out = loadmodel->lightdata;

	// MH - in my own benchmarks array indexing is actually faster than "*ptr++" and with a decent optimizing compiler why does this kind of crap even matter?
...
void *Mod_LoadAllSkins (int numskins, daliasskintype_t *pskintype)
{
	...
	for (i=0 ; i<numskins ; i++)
	{
		if (pskintype->type == ALIAS_SKIN_SINGLE)
		{
			...
			{
#ifdef NO_MIPMAPPING_LOOKS_BAD
				pheader->gltextures[i][0] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
					SRC_INDEXED, (byte *)(pskintype+1), loadmodel->name, offset, TEXPREF_NOBRIGHT | TEXPREF_MIPMAP);
				...
				pheader->fbtextures[i][0] = TexMgr_LoadImage (loadmodel, fbr_mask_name, pheader->skinwidth, pheader->skinheight,
					SRC_INDEXED, (byte *)(pskintype+1), loadmodel->name, offset, TEXPREF_FULLBRIGHT | TEXPREF_MIPMAP);
#else
				pheader->gltextures[i][0] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
			...
			for (j=0 ; j<groupskins ; j++)
			{
				...
				if (Mod_CheckFullbrights ((byte *)(pskintype), size))
				{
					...
					pheader->gltextures[i][j&3] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
						SRC_INDEXED, (byte *)(pskintype), loadmodel->name, offset, TEXPREF_NOBRIGHT | TEXPREF_MIPMAP);
					...
					pheader->fbtextures[i][j&3] = TexMgr_LoadImage (loadmodel, fbr_mask_name, pheader->skinwidth, pheader->skinheight,
						SRC_INDEXED, (byte *)(pskintype), loadmodel->name, offset, TEXPREF_FULLBRIGHT | TEXPREF_MIPMAP);
#else
					pheader->gltextures[i][j&3] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
...
#ifdef NO_MIPMAPPING_LOOKS_BAD
					pheader->gltextures[i][j&3] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
						SRC_INDEXED, (byte *)(pskintype), loadmodel->name, offset, TEXPREF_MIPMAP);
#else
					pheader->gltextures[i][j&3] = TexMgr_LoadImage (loadmodel, name, pheader->skinwidth, pheader->skinheight,
...
void * Mod_LoadSpriteFrame (void * pin, mspriteframe_t **ppframe, int framenum)
{
	...
	*ppframe = pspriteframe;
	...
	pspriteframe->gltexture =
		...
		loadmodel->name, offset, TEXPREF_ALPHA | TEXPREF_NOPICMIP | TEXPREF_MIPMAP); //johnfitz -- TexMgr
#else
	pspriteframe->gltexture =
		TexMgr_LoadImage (loadmodel, name, width, height, SRC_INDEXED, (byte *)(pinframe + 1),
