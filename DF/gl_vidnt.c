...
#ifdef DOWN_WITH_THIS_SORT_OF_THING
...
//
		...
		// and reset the mode
		Direct3D9_ResetMode (modelist[vid_default].width, modelist[vid_default].height, windowed);
...
#ifdef D3D_RESIZABLE_WINDOW
void VID_HandleWindowResize (void)
{
	RECT cr;
	...
	// unchanged
	if (vid.width == cr.right - cr.left && vid.height == cr.bottom - cr.top) return;

	// no resizing
	if (modestate == MS_FULLDIB) return;

	vid.width = cr.right - cr.left;
	vid.height = cr.bottom - cr.top;

	// in D3D changing the window size won't automatically resize the backbuffer/etc so we must reset the device instead if that happens.
	// this is effectively a mode change but for windowed modes it's quite fast so no concerns
	Direct3D9_ResetMode (vid.width, vid.height, TRUE);

	// now fill in all the ugly globals that Quake stores the same data in over and over again
	// (this will be different for different engines)
	glwidth = DIBWidth = window_width = WindowRect.right = vid.width;
	glheight = DIBHeight = window_height = WindowRect.bottom = vid.height;
...
void GL_CheckExtensions (void)
{
	...
	if (strstr(gl_extensions, "GL_ARB_texture_non_power_of_two"))
		gl_texture_npo2 = true;
	else gl_texture_npo2 = false;
#endif
...
#ifdef DIRECT3D9_WRAPPER
	// strictly speaking this is more correct for the GL case too since intel may change their vendor string
...
{
#ifdef DIRECT3D9_WRAPPER
#ifdef GUNTER_SUCKS_DONKEY_BALLS
	// retrieve the client rect each frame in case it's been changed outside of the mode-changing code; e.g by the d3d runtime clamping a mode
	if (GetClientRect (mainwindow, &WindowRect))
	{
		// only do these if they actually need to change
		if (vid.width != WindowRect.right - WindowRect.left || vid.height != WindowRect.bottom - WindowRect.top)
		{
			if (WindowRect.right - WindowRect.left > 0 && WindowRect.bottom - WindowRect.top > 0)
			{
				vid.width = WindowRect.right - WindowRect.left;
				vid.height = WindowRect.bottom - WindowRect.top;

				// now fill in all the ugly globals that Quake stores the same data in over and over again
				// (this will be different for different engines)
				DIBWidth = window_width = WindowRect.right = vid.width;
				DIBHeight = window_height = WindowRect.bottom = vid.height;

				// these also needed; vid.recalc_refdef filtering is a bit of a micro-optimaztion
				VID_UpdateWindowStatus ();
				vid.recalc_refdef = 1;

				// we assume that changes here aren't going to be excessive enough to require warpimage recalculation...
...
#endif
#ifdef SHADER_BASED_GAMMA
	// this call will succeed if:
	// (1) gamma and contrast adjustment are *NOT* needed, or,
	// (2) gamma and contrast adjustment *ARE* needed *AND* the D3D objects to support them were created successfully
	// otherwise it will fail.
	// so the *ONLY* failure case is (1) D3D failed to create a rendertarget at the scrrent screen res, and (2) D3D
	// failed to create it's shaders.
	if (!Direct3D9_SetupGammaAndContrast (vid_gamma.value, 1.0f))
	{
...
#ifdef DIRECT3D9_WRAPPER
    if ( (pixelformat = Direct3D9_ChoosePixelFormat(hDC, &pfd)) == 0 )
#else
...
#ifdef DIRECT3D9_WRAPPER
	Direct3D9_DescribePixelFormat(hDC, pixelformat, sizeof(PIXELFORMATDESCRIPTOR), &test); //johnfitz
#else
	DescribePixelFormat(hDC, pixelformat, sizeof(PIXELFORMATDESCRIPTOR), &test); //johnfitz
#endif

#ifdef DIRECT3D9_WRAPPER
    if (Direct3D9_SetPixelFormat(hDC, pixelformat, &pfd) == FALSE)
#else
...
#ifdef DIRECT3D9_WRAPPER
	// d3d automatically handles ChangeDisplaySettings/etc for us
	if (fActive)
	{
