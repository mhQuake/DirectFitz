...
void R_DrawSequentialPoly (msurface_t *s)
{
	...
	// FitzQuake doesn't make this as easy as it should be; a nicer engine would just send bmodels through the same chain drawer as the world
...
// instanced model
	if (clmodel->firstmodelsurface != 0 && !gl_flashblend.value)
	{
		for (k=0 ; k<MAX_DLIGHTS ; k++)
		{
			...
			VectorSubtract (cl_dlights[k].origin, e->origin, cl_dlights[k].transformed);
			...
			if (e->angles[0] || e->angles[1] || e->angles[2])
			{
				...
				VectorCopy (cl_dlights[k].transformed, temp);
				AngleVectors (e->angles, forward, right, up);

				cl_dlights[k].transformed[0] = DotProduct (temp, forward);
				cl_dlights[k].transformed[1] = -DotProduct (temp, right);
				cl_dlights[k].transformed[2] = DotProduct (temp, up);
			}
#endif
	...
	for (lnum=0 ; lnum<MAX_DLIGHTS ; lnum++)
	{
		...
		for (t = 0 ; t<tmax ; t++)
		{
			...
			for (s=0 ; s<smax ; s++)
			{
				...
				{
#ifdef EXTRA_DLIGHTS
					bl[0] += (int) ((rad - dist) * cl_dlights[lnum].rgb[0]);
					bl[1] += (int) ((rad - dist) * cl_dlights[lnum].rgb[1]);
					bl[2] += (int) ((rad - dist) * cl_dlights[lnum].rgb[2]);
#else
					brightness = rad - dist;
...
store:
	...
	case GL_RGB:
		...
		for (i=0 ; i<tmax ; i++, dest += stride)
		{
			...
			{
#ifdef RGB_2X_IS_NOT_ENOUGH
				if (gl_overbright.value && gl_texture_env_combine && gl_mtexable)
				{
					t = *bl++ >> 9;if (t > 255) t = 255;*dest++ = t;
					t = *bl++ >> 9;if (t > 255) t = 255;*dest++ = t;
					t = *bl++ >> 9;if (t > 255) t = 255;*dest++ = t;
				}
				else
...
void R_UploadLightmap(int lmap)
{
#ifndef DYNAMIC_LIGHT_OPTIMIZATION
	glRect_t	*theRect;
