...
===============
*/
#ifndef FITZQUAKE_SKY_GIVES_ME_FITS
...
#ifdef FITZQUAKE_SKY_GIVES_ME_FITS
// declared this way so that we can pass glVertex3fv as a Sky_DrawFunc
void APIENTRY Sky_DrawMultitexture (float *v)
{
	float s, t;
...
void APIENTRY Sky_DrawSolidLayer (float *v)
{
	float s, t;
...
void APIENTRY Sky_DrawAlphaLayer (float *v)
{
	float s, t;
...
void Sky_DrawPass (int axis, void (APIENTRY *Sky_DrawFunc) (float *))
{
	int i, j;
...
void Sky_DrawFace (int axis)
{
	...
	if (gl_mtexable && r_skyalpha.value >= 1.0)
	{
		...
		Sky_DrawPass (axis, Sky_DrawMultitexture);
	...
	else
	{
		...
		Sky_DrawPass (axis, Sky_DrawSolidLayer);
		...
		Sky_DrawPass (axis, Sky_DrawAlphaLayer);
